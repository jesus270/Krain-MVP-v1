...
export async function GET(request: NextRequest) {
  try {
    ...
    log.info("Getting user data for whitelist app", {
      operation: "get_user",
      entity: "API",
      app: "whitelist",
      userId,
    ...
    if (!mergedUser) {
      log.error("Failed to get merged user data for whitelist app", {
        operation: "get_user",
        entity: "API",
        app: "whitelist",
        userId,
    ...
    log.info("Successfully retrieved user data for whitelist app", {
    ...
    const privyId: string | undefined =
      ...
      request.headers.get("X-User-Id") ||
      undefined;
    log.error("Failed to get user data for whitelist app", {
